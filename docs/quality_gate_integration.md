...
The Quality Gate is a **prerequisite step** that runs before cognitive scoring in the CI/CD pipeline. It ensures fundamental code quality standards are met before consuming resources on more expensive analyses.
...
PR Created/Updated
       ↓
   Quality Gate Analysis ← RUNS FIRST
       ↓
...
Edit `src/scoring/quality_gate.py` to adjust:
